...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        void ScanDoors()
        {
            GridTerminalSystem.GetBlocksOfType<IMyDoor>(null, ScanDoorsCollect);
        }

        private const string kClosingData = "Closing";

        bool ScanDoorsCollect(IMyTerminalBlock block)
        {
            if (!Me.IsSameConstructAs(block)) return false;
            if (block is IMyAirtightHangarDoor) return false;

            var door = (IMyDoor)block;
            if (door.Status == DoorStatus.Open)
            {
                if (door.CustomData == kClosingData)
                {
                    door.CustomData = string.Empty;
                    door.CloseDoor();
                }
                else
                {
                    door.CustomData = kClosingData;
                }
            }
            return false;
        }
